...
$tartalom = szerkezet();
  ...
  function csoportForm(){
    ...
    return '
    <form method="post" action="?page=csoport">
  <div class="container">
  ...
  foreach($csoportListaAdat as $egyCsoport){
    ...
    $vissza.="<li class=\"list-group-item$elemClass\">$egyCsoport[nev]
                <a href=\"?page=csoport&action=edit&id=$egyCsoport[id]\" class=\"$linkColor\"><i class=\"bi bi-pencil\"></i></a>
                <a href=\"?page=csoport&action=delete&id=$egyCsoport[id]\ class=\"$linkColor\"><i class=\"bi bi-trash\"></i></a>
              </li>";
  }
...
';
  ...
  function csoportListaAdat(){
    ...
    $query="SELECT * from csoport WHERE ?";
    $vissza=[];
  ...
  function csoportAdat($id){
    ...
    $query="SELECT * from csoport WHERE id=?";
    $vissza=[];
    if($stmt = $conn->prepare($query)) {
  ...
  function csoportInsert($nev){
    ...
    $query="INSERT INTO csoport (nev) values (?)";
    if($stmt = $conn->prepare($query)) {
  ...
  function csoportUpdate($id,$nev){
    ...
    $query="UPDATE csoport SET nev=? WHERE id=?";
    if($stmt = $conn->prepare($query)) {
  ...
  function csoportDelete($id)
  {
    ...
    $query="DELETE FROM csoport WHERE id=?";
    if($stmt = $conn->prepare($query)) {
